        ...
        <View style={{ justifyContent: 'flex-start' }}>
            ...
            <View style={{
                ...
                <TouchableOpacity
                    ...
                    contentStyle={{ height: hp("7.7%"), borderColor: '#FFF'}}
                    //labelStyle={{color: "#000"}}
                    mode='outlined'
                    onPress={() => this.setState({isDatePickerOpen: true})}
        ><Text style={{ color: 'rgba(0, 0, 0, .6)', fontSize: 16  }}>{this.state.date.getDate()+'/'+this.state.date.getMonth()+'/'+ this.state.date.getFullYear()}</Text></TouchableOpacity>
                <Menu
                    visible={this.state.isGenderMenuOpen}
                    onDismiss={() => this.setState({isGenderMenuOpen: false})}
                    anchor={
                        <TouchableOpacity
                            ...
                            //labelStyle={{color: "#000"}}
                            mode='outlined'
                            onPress={() => this.setState({isGenderMenuOpen: true})}
                        ><Text style={{ color: 'rgba(0, 0, 0, .6)', fontSize: 16 }}>{this.state.genderValue}</Text></TouchableOpacity>
                    }
                >
                    <Menu.Item onPress={() => this.setState({genderValue: "Masculino", isGenderMenuOpen: false})} title="Masculino" />
                    <Menu.Item onPress={() => this.setState({genderValue: "Feminino", isGenderMenuOpen: false})} title="Feminino" />
                </Menu>
            </View>
             <TextInput
                style={styles.LastnameInput}
